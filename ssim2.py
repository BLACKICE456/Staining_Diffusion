...
class SSIM(nn.Module):
    ...
    def forward(self, img1: torch.Tensor, img2: torch.Tensor) -> torch.Tensor:
        ...
        sqrt_sigma1_sq = torch.sqrt(sigma1_sq)
        sqrt_sigma2_sq = torch.sqrt(sigma2_sq)

        luminance = (2 * mu1_mu2 + self.C1) / (mu1_sq + mu2_sq + self.C1)
        contrast = (2 * sqrt_sigma1_sq * sqrt_sigma1_sq + self.C2 ) / (sigma1_sq + sigma2_sq + self.C2)
        structure = (2 * sigma12 + self.C3) / (sqrt_sigma1_sq * sqrt_sigma2_sq + self.C3)
        ...
        if self.mode == "structure":
            ...
            ssim_map = structure
        elif self.mode == "structure_contrast":
            ssim_map = structure = structure + (1-contrast)
        else:
            raise ValueError("There is no mode of {}".format(self.mode))
